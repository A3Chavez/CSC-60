...
int rotate_left(unsigned num, int n)
{
	...
	{
			bit = (num & mask) ? 1 : 0;  // Set/Save the bit = 1 | 0 depending on result of nbits/mask
			num <<= 1;  // Left shift the 'num' by one
			num |= bit;  // Add the isolated bit in 'bit' onto the right of num
	}
	return num;
}
