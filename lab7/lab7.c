		...
		stats_t *race_stats )             /* in & out */
{
	...
	for(d = 0; d < NRACERS; d++) 
	{
		...
		for(t = 0; t < TRIES; t++)
		{
			if(driver_list[d].best_time > driver_list[d].tries[t])
	...
	race_stats -> best_average = race_stats -> best_average / NRACERS;
		...
		stats_t *race_stats )
{
	...
	int mid_p = NRACERS / 2;
	if(mid_p % 2 != 0)
		race_stats -> median = driver_list[mid_p].best_time;
	else 
		race_stats -> median = (driver_list[mid_p].best_time + driver_list[mid_p - 1].best_time) / (2.0);
	return;
}
